...
public class ComboFeature extends JComboBox {
	...
	public ComboFeature(String init) {
		super();
		//if (features[Parameters.GRADIENT_HORIZONTAL]) 
		addItem("Gradient-X");
		//if (features[Parameters.GRADIENT_VERTICAL]) 
		addItem("Gradient-Y");
		//if (features[Parameters.GRADIENT_MODULUS]) 
		addItem("Orientation");
		//if (features[Parameters.TENSOR_COHERENCY]) 
		addItem("Coherency");
		//if (features[Parameters.TENSOR_ENERGY] & !angle)
		addItem("Energy");
		//if (features[Parameters.TENSOR_HARRIS] & !angle) 
		addItem("Constant");
